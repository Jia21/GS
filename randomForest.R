...
for (threshold in 1:length(thresholds)){
  for(trait in 1:length(traits)){
    ...
    for(iter in 1:1000){
      ...
      set.seed(131);
      pheno_trait_res = randomForest(target ~ .,data = poplar_trait,subset = train,ntree=500);
    ...
    write.table(predict_accuracy_iter,paste("predict_accuracy_iter_",traits[trait],"_",thresholds[threshold],"_RF",sep=""),quote = F,sep="\t",col.names = F,row.names = F);
    write.table(predict_accuracy_iter_bias,paste("predict_accuracy_iter_bias_",traits[trait],"_",thresholds[threshold],"_RF",sep=""),quote = F,sep="\t",col.names = F,row.names = F);
    write.table(bias_final,paste("bias_final_",traits[trait],"_",thresholds[threshold],"_RF",sep=""),quote = F,sep="\t",col.names = F,row.names = F);
    write.table(predict_accuracy_pval_iter,paste("predict_accuracy_pval_iter_",traits[trait],"_",thresholds[threshold],"_RF",sep=""),quote = F,sep="\t",col.names = F,row.names = F);
  }
}
